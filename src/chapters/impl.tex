...
will be empty(no interfaces). After the device is started the user can proceed to issue commands( see \textbf{Appendix} 
\ref{chapter:commands} for a complete listing of available commands) to configure the device. Starting a large number of
...
\lstset{caption=Opening a tun/tap interface,label=lst:tapopen}
