...
A network driver is a driver that commands a NIC. Because \textbf{\project} does not use physical interfaces
the driver used by \textbf{\project} is a purely virtual one. As all drivers implemented in \textbf{LKL} 
it is split into two parts, a kernel part(LKL) and a native part. Communication between the two parts
